...
data-select-countries - додається до пустого селекту, в якому будуть відображені караїни з об'єкта all-countries
...
class SelectConstructor {
  constructor(props, data = null) {
    ...
    if (this.config.init) {
      ...
      const selectItems = data
        ...
        : document.querySelectorAll('.wpcf7-select')
  ...
  getOption(selectOption, originalSelect) {
    const { classSelectOption, classSelectOptionSelected } = this.selectClasses
    const {
      selected,
      value,
    ...
    const selectOptionSelected =
      selected && isMultiple ? ` ${classSelectOptionSelected}` : ''
    ...
    const selectOptionLinkTarget =
      optionHref && selectOption.hasAttribute('data-href-blank')
        ? 'target="_blank"'
        : ''

    const content = this.getSelectElementContent(selectOption)
    const flagElement = optionFlag
      ? `<span class="fi fi-${optionFlag}"></span>`
      : ''
