...
function resolveAssetPath(outputDir, fromFile, url) {
   ...
   if (cleanUrl.startsWith('/')) {
      return path.resolve(outputDir, cleanUrl.slice(1))
   }

   return path.resolve(path.dirname(fromFile), cleanUrl)
}

async function updateCssFiles(outputDir, generatedWebPFiles) {
   const cssFiles = []
   walkDir(outputDir, (filePath) => {
               ...
               Declaration: (decl) => {
                  if (decl.value.includes("url(")) {
                     decl.value = decl.value.replace(
                        ...
                        (match, quote, url) => {
                           const newUrl = url.replace(/\.(jpe?g|png)(?=([?#]|$))/i, '.webp')
                           const absoluteWebpPath = resolveAssetPath(outputDir, cssFile, newUrl)
                           if (!absoluteWebpPath || !generatedWebPFiles.has(absoluteWebpPath)) {
                              return match
                           }
                           return `url('${newUrl}')`
                        }
   ...
   return {
      ...
      async closeBundle() {
         ...
         if (generatedWebPFiles.size > 0) {
            ...
            await updateCssFiles(resolvedOutputDir, generatedWebPFiles)
         }
      },
   }
}
